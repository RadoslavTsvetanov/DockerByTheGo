...
app.post(
  ...
  async (
    req: express.Request<
      {},
      { payload: { handlerId: number; content: string } }
    >,
...
app.post(
  "/channel/new",
  async (
    req: express.Request<{}, CustomHandlerInfo>,
    res: express.Response
  ) => {
    const client = new YourServiceClient(
      ENV.getGrpcWorkloadServerUrl(),
    ...
    client.createWorload(
      { channelId: req.body.channelId },
      (error: ServiceError | null, response: WorkloadResponse) => {
        req.body;
        if (error) {
          error;
          exit();
